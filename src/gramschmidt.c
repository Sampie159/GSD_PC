...
#pragma scop
  for (k = 0; k < _PB_N; k++) { // Lê todos os vetores
    nrm = SCALAR_VAL(0.0);
    for (i = 0; i < _PB_M; i++)
      nrm += A[i][k] *
             A[i][k]; // Normal é a soma dos quadrados de cada elemento do vetor
    R[k][k] = SQRT_FUN(nrm); // R[k][k] guarda a raiz da normal
    for (i = 0; i < _PB_M; i++)
      Q[i][k] = A[i][k] / R[k][k];
    for (j = k + 1; j < _PB_N; j++) {
...
int main(int argc, char **argv) {
  ...
  if (programa.num_threads == 1) {
    ...
    pthread_barrier_init(&barreira, NULL, programa.num_threads);
    pthread_barrier_init(&barreira2, NULL, programa.num_threads);
    pthread_spin_init(&sl, programa.num_threads);

    pthread_t threads[programa.num_threads];
    int qt_linha_thread = M / programa.num_threads;

    for (int i = 0; i < programa.num_threads; i++) {
      Args *args = (Args *)malloc(sizeof(Args));
      args->inicio = qt_linha_thread * i;
      args->fim =
          (i == programa.num_threads - 1) ? M : args->inicio + qt_linha_thread;
...
static void print_ajuda(void) {
  printf(
      ...
      "-h:                                  - Escreve essa mensagem de ajuda.");
}
